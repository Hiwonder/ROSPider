...
class FacialExpressionNode:
    def __init__(self, name, log_level=rospy.INFO):
        ...
        self.detect_w, self.detect_h = 640, 360 # 识别处理过程的图片尺寸
    ...
    def image_process(self):
        ...
        rgb_image = cv2.resize(rgb_image, (self.detect_w, self.detect_h)) # 缩放图像
        ...
        if time.time() > self.timestamp and self.count > 30 and self.expression[0] != "":  
            if self.expression[0] == "Angry":
                self.rospider.run_action_set("/home/hiwonder/ActionSets/attack.d6a", 1)
                self.timestamp = time.time() + 10
            elif self.expression[1] == "Happy":
                self.rospider.run_action_set("/home/hiwonder/ActionSets/twist_l.d6a", 1)
                self.timestamp = time.time() + 3
            elif self.gesture == "Neutral":
                self.rospider.run_action_set("/home/hiwonder/ActionSets/wave.d6a", 1)
