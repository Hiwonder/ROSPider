...
import signal
import numpy as np
import vision_utils.fps as fps
import vision_utils
from sensor_msgs.msg import Image
from rospider_interfaces.msg import ColorInfo, ColorsInfo
from rospider_interfaces.srv import SetColorDetectParam, SetCircleROI, SetLineROI
...
class ColorDetectNode:
    def __init__(self, name):
        ...
        self.lab_data = vision_utils.get_yaml_data("/home/hiwonder/rospider/src/lab_config/config/lab_config.yaml")
        ...
        self.camera_sub = rospy.Subscriber(self.camera_rgb_prefix + '/image_raw', Image, self.image_callback, queue_size=1)
        ...
        while self.running:
            if self.image is not None and self.target_colors is not None:
                ...
                for i in self.target_colors:
                    if self.detect_type[i] == 'line' and i != '':
                        ...
                        for roi in self.line_roi:
                            ...
                            max_contour_area = vision_utils.get_area_max_contour(contours, 0)  # 获取最大面积对应轮廓
                            if max_contour_area is not None:
                                rect = cv2.minAreaRect(max_contour_area[0])  # 最小外接矩形
                                box = np.int0(cv2.boxPoints(rect))  # 四个角
                                for j in range(4):
                                    box[j, 1] = box[j, 1] + roi_value[0]
                                cv2.drawContours(self.result_image, [box], -1, vision_utils.range_rgb[i], 2)  # 画出四个点组成的矩形
                                ...
                                cv2.circle(self.result_image, (int(line_center_x), int(line_center_y)), 5, vision_utils.range_rgb[i], -1)  # 画出中心点
                ...
                if areaMaxContour_rect != 0:
                    ...
                    cv2.drawContours(self.result_image, [box], -1, vision_utils.range_rgb[color_area_max_rect], 2)  # 画出四个点组成的矩形
                ...
                if areaMaxContour_circle != 0:
                    ...
                    radius = int(radius)            
                    cv2.circle(self.result_image, (x, y), radius, vision_utils.range_rgb[color_area_max_circle], 2)  # 画圆
                ...
                self.result_publisher.publish(vision_utils.cv2_image2ros(result_image, self.name))
            else:
                rospy.sleep(0.01)
