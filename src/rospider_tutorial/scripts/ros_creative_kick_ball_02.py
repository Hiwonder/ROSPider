...
class ColorKickNode:
    def __init__(self, target_color, log_level=rospy.INFO):
        ...
        self.pid_yaw = pid.PID(0.4, 0.01, 0.01) # 控制偏航角的 pid 控制器
        ...
        rospy.sleep(1)
        self.rospider.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
        self.step = 0
    ...
    def image_callback(self, ros_image: Image):
        ...
        try:
            ...
            if max_contour_area is not None:
                ...
                if self.step == 0 and self.count > 60: # 让机器人正对小球
                    # 画面 y 轴, 控制俯仰
                    ...
                    if abs(self.yaw - 1500) > 5:
                        ...
                        print(out)
                        self.rospider.set_step_mode(1, 0, 10, 0, -math.radians(out), 0.6, repeat=0)
                    else:
                        self.step = 1
                        self.rospider.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
                        print(self.step)

                elif self.step == 1: # 左右平移让小球到达合适位置
                    if abs(center_x  - 200) > 10:
                        ...
                        self.rospider.set_step_mode(1, abs(out), 10, math.radians(d), 0, 0.6, repeat=0)
                    else:
                        self.step = 2
                        self.rospider.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
                elif self.step == 2: # 前后移动让小球到达合适位置
                    if abs(center_y - (h/2)) > 20: # 相差范围小于一定值就不用再动了
                        self.pid_pitch.SetPoint = h/2 # 我们的目标是要让色块在画面的中心, 就是整个画面的像素宽度的 1/2 位置
                        self.pid_pitch.update(center_y) # 更新 pid 控制器
                        self.pitch += self.pid_pitch.output # 获得 pid 输出
                        pwm_servo.pwm_servo2.set_position(self.pitch, 33)
                    ...
                    if abs(self.pitch - 1200) > 10:
                        ...
                        self.rospider.set_step_mode(1, out, 10, math.radians(d), 0, 0.6, repeat=0)
                    else:
                        self.step = 3
                        self.rospider.stop_running()
                        self.timestamp = time.time()
                        print(self.step)
                elif self.step == 3: # 踢球
                    if time.time() - self.timestamp > 1:
                        try:
                            self.rospider.run_action_set("/home/hiwonder/ActionSets/kick.d6a", repeat=1)
                    ...
                    if time.time() - self.timestamp > 5:
                        self.step = 0
                        self.rospider.set_step_mode(-2, 0, 10, 0, 0, 0.6, repeat=0)
            else:
                self.step = 0
                self.pid_yaw.clear()
                self.pid_pitch.clear()
