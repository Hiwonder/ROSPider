...
class ARNode:
    def __init__(self, name):
        ...
        # 加载 3d 文件
        model_dir = os.path.join(rospkg.RosPack().get_path("rospider_tutorial"), 'misc/3d_model')
    ...
    def image_proc(self, rgb_image, result_image):
        ...
        if detections != ():
            for detection in detections:
                ...
                # 将缩放后的识别到的点坐标缩放回原图
                lb, rb, rt, lt = [point_remapped(c, (int(o_width/2), int(o_height/2)), (o_width, o_height)) for c in tag_corners[:4]]
