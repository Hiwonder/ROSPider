...
class FingerTrackNode:
    ...
    #    rospy.loginfo('Received an image! ')
        ...
        try:
            results = self.hand_detector.process(rgb_image) # 进行识别
                
            if results is not None and results.multi_hand_landmarks: # 有识别到手部关键点
                for hand_landmarks in results.multi_hand_landmarks:
                    self.drawing.draw_landmarks( # 画出手部
                        result_image,
                        hand_landmarks,
                        mp.solutions.hands.HAND_CONNECTIONS)
                    landmarks = get_hand_landmarks(rgb_image, hand_landmarks.landmark)
                    self.points.append(landmarks[8].tolist()) # 保存食指指尖轨迹
                self.timestamp = time.time()
            else:
                if self.timestamp - time.time() > 2: # 超过一定时间没有识别到手就清空点列
                    self.points = []
