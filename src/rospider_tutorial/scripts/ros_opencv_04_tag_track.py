...
class TagNode:
    ...
    def image_proc(self, rgb_image, result_image):
        ...
        gray = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2GRAY) # 将 RGB 图片转为灰度图
        gray = cv2.resize(gray, (int(o_width/2), int(o_height/2))) 
        detections = self.tag_detector.detect(gray)
        if detections:
            for detection in detections: # 遍历所有识别到的标签
                tag_id = detection['id']
                tag_center = detection['center']
                tag_corners = detection['lb-rb-rt-lt']
                corners =[point_remapped(p, (int(o_width/2), int(o_height/2)), (o_width, o_height)) for p in tag_corners]
                lb, rb, rt, lt = corners
                ...
                tag_center = point_remapped(tag_center,(int(o_width/2), int(o_height/2)), (o_width, o_height))
        ...
        if self.tag is not None:
            ...
            if abs(w - 20) > 2:
                ...
                print(self.speed)
                if self.speed > 0:
                    self.rospider.set_step_mode(1, abs(self.speed), 10, 0, 0, 0.6, repeat=0)
                elif self.speed < 0:
                    self.rospider.set_step_mode(1, abs(self.speed), 10, math.radians(180), 0, 0.6, repeat=0)
                else:
                    self.rospider.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
        else:
            self.rospider.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
