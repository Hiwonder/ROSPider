...
class FaceDetectNode:
    ...
    def image_callback(self, ros_image):
        ...
        org_image = np.copy(rgb_image)
        black_image = np.zeros((ros_image.height, ros_image.width, 3), dtype=np.uint8)
        #result_image = np.zeros(ros_image.height, ros_image.width * 2, 3)#np.copy(rgb_image) # 拷贝一份用作结果显示，以防处理过程中修改了图像
        rgb_image = cv2.resize(rgb_image, (320, 180), cv2.INTER_NEAREST) # 缩放图片
        try:
            results = self.face_mesh.process(rgb_image) # 调用人脸检测
        ...
        result_image = np.concatenate([org_image, black_image], axis=1)
        self.fps.update()
