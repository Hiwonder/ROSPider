...
class ObjectronNode:
    ...
    def image_callback(self, ros_image):
        ...
        result_image = np.copy(cv2.resize(rgb_image, (int(org_w * 1.6), int(org_h * 1.6)))) # 拷贝一份用作结果显示，以防处理过程中修改了图像
        rgb_image = cv2.resize(rgb_image, (int(org_w / 4), int(org_h/4)), cv2.INTER_NEAREST) # 缩放图片
        try:
            results = self.objectron.process(rgb_image)
            if results.detected_objects: #  有识别到
