...
class Board:
    ...
    def __init__(self, device="/dev/rrc", baudrate=1000000, timeout=5):
        ...
        time.sleep(0.5)
        self.recv_stop = threading.Event()  # 控制线程退出的标志
        self.recv = threading.Thread(target=self.recv_task, daemon=True)
        self.recv.start()
        # time.sleep(0.5)
...
def bus_servo_test(board):
    board.bus_servo_set_position(2, [[1, 500]])
    time.sleep(3)

    board.bus_servo_set_position(2, [[1, 700]])
    time.sleep(3)
# def bus_servo_test(board):
