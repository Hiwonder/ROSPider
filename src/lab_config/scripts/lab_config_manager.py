...
class LabConfigManagerNode:
    ...
    def image_callback(self, ros_image):
        ...
        # 对图像进行处理做二值化等操作
        #image_resize = cv2.resize(image, (ros_image.width, ros_image.height), interpolation=cv2.INTER_NEAREST)
        frame_result = cv2.cvtColor(image, cv2.COLOR_RGB2LAB)
        ...
        # 将处理后的二值化图像发布
        #dilated = cv2.resize(dilated, (ros_image.width , 360))
