...
from rospider_sdk import misc, buzzer
...
class JoystickControlNode:
    ...
    def start_callback(self, new_state):
        if new_state == ButtonState.Pressed:
            ...
            self.rospider.traveling(gait=-2, time=1, steps=0)
    ...
    def joy_callback(self, event):
        ...
        #    self.rospider.traveling(10 + self.gait, height=self.step_height, time=self.period)
        #    self.set_parameters = False
        ...
        if self.do_movement:
            ...
            self.rospider.cmd_vel(vx, vy, rotate)
            self.do_movement = False

        if self.do_transform:
            self.rospider.pose_transform_euler(self.transform[0], self.transform[1], 0.1)
            self.transform = [[0, 0, 0], [0, 0, 0]]
            self.do_transform = False
