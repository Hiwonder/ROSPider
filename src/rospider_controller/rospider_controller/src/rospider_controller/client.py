import rospy
from rospider_controller_interfaces.msg import Traveling, TransformEuler, LegPosition, TransformEuler, RunActionSet, Pose
from rospider_controller_interfaces.srv import SetPose1, SetPose1Request
...
class Client:
    def __init__(self, node):
        self.node = node
        self.traveling_pub = rospy.Publisher("rospider_controller/traveling", Traveling, queue_size=1)
        self.head_absolute_pub = rospy.Publisher("rospider_controller/set_head_absolute", TransformEuler, queue_size=1)
        self.head_relatively_pub = rospy.Publisher("rospider_controller/set_head_relatively", TransformEuler, queue_size=1)
        self.leg_absolute_pub = rospy.Publisher("rospider_controller/set_leg_absolute", LegPosition, queue_size=1)
        self.leg_relatively_pub = rospy.Publisher("rospider_controller/set_leg_relatively", LegPosition, queue_size=1)
        self.transform_euler_pub = rospy.Publisher("rospider_controller/pose_transform_euler", TransformEuler, queue_size=1)
        self.cmd_vel_pub = rospy.Publisher("rospider_controller/cmd_vel", Twist, queue_size=1)
        self.run_actionset_pub = rospy.Publisher("rospider_controller/run_actionset", RunActionSet, queue_size=1)
        self.set_build_in_pose_proxy = rospy.ServiceProxy("rospider_controller/set_pose_1", SetPose1)
        self.set_pose_euler_sub = rospy.Publisher("rospider_controller/set_pose_euler", Pose, queue_size=1)
