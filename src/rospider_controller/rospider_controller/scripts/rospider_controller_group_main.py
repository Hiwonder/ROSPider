...
class rospiderControlNode:
    def __init__(self, node_name):
        ...
        # 机器人姿态设置服务
        self.set_pose1_srv = rospy.Service("/rospider_controller/set_pose_1", SetPose1, self.set_pose1_cb) # 通过姿态名称设置机器人的姿态， "DEFAULT_POSE" "DEFAULT_POSE_M"
        self.set_pose2_srv = rospy.Service("/rospider_controller/set_pose_2", SetPose2, self.set_pose2_cb) # 通过六条腿的末端坐标设置机器人的姿态

        # 机器人机体姿态变换
        # 通过 平移和欧拉角旋转变换机器人的姿态， 相对姿态变换, 旋转顺序为 RPY
        self.set_transform2_sub = rospy.Subscriber("/rospider_controller/pose_transform_euler", jetmsg.TransformEuler, self.pose_transform_euler_cb)
        # 通过 平移和欧拉角旋转变换机器人的姿态， 绝对变换, 旋转顺序为 RPY
        self.set_pose_euler_sub = rospy.Subscriber("/rospider_controller/set_pose_euler", jetmsg.Pose, self.set_pose_euler_cb)
        # 设置一条腿末端移动到指定位置, 绝对坐标
        self.set_leg_position_sub = rospy.Subscriber("/rospider_controller/set_leg_absolute", jetmsg.LegPosition, self.set_leg_absolute_cb)
        # 设置一条腿末端移动当相对与当期位置的指定位置
        self.set_leg_position_re_sub = rospy.Subscriber("/rospider_controller/set_leg_relatively", jetmsg.LegPosition, self.set_leg_relatively_cb)

        # 机器人头部姿态控制
        # 绝对旋转，RPY=0, 0, 0时 云台水平正视前方
        self.set_head_absolute = rospy.Subscriber("/rospider_cotnroller/set_head_absolute", jetmsg.TransformEuler, self.head_absolute_cb)
        # 相对旋转
        self.set_head_relatively = rospy.Subscriber("/rospider_controller/set_head_relatively", jetmsg.TransformEuler, self.head_relatively_cb)

        # 机器人移动服务
        # 通过步态参数控制机器人的移动
        self.traveling = rospy.Subscriber("/rospider_controller/traveling", jetmsg.Traveling, self.set_traveling_cb)
        # 通过线速度、角速度控制机器人的移动，其他参数由上一次执行的 gait大于0的traveling来指定
        self.cmd_vel_sub = rospy.Subscriber("/rospider_controller/cmd_vel", geometry_msgs.msg.Twist, self.controller.cmd_vel)

        # 机器人动作组运行服务
        self.run_action_sub = rospy.Subscriber("/rospider_controller/run_actionset", jetmsg.RunActionSet, self.run_action_set_sub_cb)
